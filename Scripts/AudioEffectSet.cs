...
#if ODIN_INSPECTOR_3
        [BoxGroup("Info"), Tooltip("The set's tag, used to access the effect set in code.")] public AudioEffectTag tag = new AudioEffectTag();
...
#else
        [Tooltip("The set's tag, used to access the effect set in code.")] public AudioTag tag = new AudioTag();
        [Tooltip("Should all of the effects in this set be loaded automatically?")] public bool loadOnLaunch = false;
        [Tooltip("The audio mixer group to output to.  Effect data may override this value.")] public AudioMixerGroup mixerGroup = null;
